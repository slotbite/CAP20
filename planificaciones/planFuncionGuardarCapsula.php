<?php
session_start();

include("../default.php");
require('clases/planificaciones.class.php');
$objPlanificacion = new planificaciones();

$clienteId = $_SESSION['clienteId'];
$usuarioModificacion = $_SESSION['usuario'];

if ($clienteId == '') {
    echo "<script>window.location='../index.php';</script>";
}

$planificacionId = mb_convert_encoding(trim($_POST['planificacionId']), "ISO-8859-1", "UTF-8");
$capsulaId = mb_convert_encoding(trim($_POST['capsulaId']), "ISO-8859-1", "UTF-8");
$fechaEnvio = mb_convert_encoding(trim($_POST['fechaEnvio']), "ISO-8859-1", "UTF-8");


$consulta = $objPlanificacion->planGuardarCapsula($planificacionId, $clienteId, $capsulaId, $fechaEnvio, $usuarioModificacion);
        
if ($consulta) {

    $resultado = mssql_fetch_array($consulta);
    echo $resultado['estado'];
}
?>
...
class planificaciones {
    ...
    function planMostrarPlanificacionCapsulas($planificacionId) {
        $query = "  Select  t.temaNombre, 
                            CASE WHEN c.capsulaNumero is NULL or LTRIM(RTRIM(c.capsulaNumero)) = '' THEN c.capsulaNombre ELSE LTRIM(RTRIM(c.capsulaNumero)) + '.- ' + c.capsulaNombre END as capsulaNombre, 
                            c.capsulaVersion, cp.fechaEnvio, cp.fechaCierre, 
                            (CASE WHEN e.cantidad is not NULL THEN 'Enviada: ' + CAST(e.cantidad as varchar) + ' envío(s).' ELSE CASE WHEN cp.fechaEnvio < '01-04-2014' THEN 'Enviada' ELSE CASE WHEN DATEADD(day, 1, cp.fechaEnvio) >= GETDATE() THEN 'En espera' ELSE 'Anulada' END END END) as 'estado', 
                            c.capsulaId
                    ...
                    inner join Temas t (nolock) on c.temaId = t.temaId
                    left join (	Select planificacionId, capsulaId, capsulaVersion, COUNT(*) as cantidad
                                            From Envios
                                            Where planificacionId is not null
                                            Group by planificacionId, capsulaId, capsulaVersion) e on cp.capsulaId = e.capsulaId and cp.capsulaVersion = e.capsulaVersion and cp.planificacionId = e.planificacionId

...
if ($evaluacionId == '') {
    echo "<script>window.location='../evaluaciones/evaEvaluaciones.php';</script>";
}
...
$evaluacionUsuarios = $objEvaluacion->evaMostrarEvaluacionUsuarios($evaluacionId);
?>      
...
<script type="text/javascript">
    ...
    window.addEvent('domready', function() {
        ...
        $('temaNombre').disabled = true;
        $('lupaTema').setStyle('display', 'none');
...
<input type="hidden" name="evaluacionEstado" id="evaluacionEstado" value="<?php echo htmlentities($evaluacion['evaluacionEstado']) ?>"/>
    ...
    <div class="divFormulario">
        <table>  
            ...
            <tr>
                ...
                <td><input id="temaNombre" name="temaNombre" type="text" class="campo" value="<?php echo htmlentities($evaluacion['temaNombre']) ?>"/></td>                   
                <td>
                    <img id="lupaTema" src='../skins/saam/img/lupa.png' style="cursor: pointer" onclick="evaSeleccionarTemas()"/>                
                </td>
                <td Valign="top">
                    <label id="oTema" style="color: #FF0000; font-size: 12px">(*)</label>
                    <div id ="mBoxContainerTema" class="BoxContainer" style="position: absolute;opacity:0;filter:alpha(opacity=0);" height="auto">
                        ...
                        <div id="BoxContentTema" class="BoxContent " style="width: auto; height: auto;">                        
            ...
            <tr>
                ...
                <td><input id="evaluacionNombre" name="evaluacionNombre" type="text" class="campo" style="text-transform:uppercase;" maxlength="50" value="<?php echo htmlentities($evaluacion['evaluacionNombre']) ?>"/> </td>                    
                <td></td>
                <td><label id="oEvaluacion" style="color: #FF0000; font-size: 12px">Debe escribir un nombre para la Evaluaci&oacute;n</label></td>
            </tr>
            <tr>
                <td>Descripción: </td>
                <td colspan="3"><input id="evaluacionDescripcion" name="evaluacionDescripcion" type="text" class="campoDescripcion" maxlength="250" value="<?php echo htmlentities($evaluacion['evaluacionDescripcion']) ?>"/></td>  
                <td></td>
                <td></td>            
            </tr>                
        </table>                        
            ...
            <tr>
                ...
                <td width="500px" valign="top">                
                    ...
                    <div id ="mBoxContainer" class="BoxContainer" style="position: absolute; left: 400px;opacity:0;filter:alpha(opacity=0);" height="auto">
                        ...
                        <div id="BoxContent" class="BoxContent " style="width: auto; height: auto;">                        
            ...
            <tr>
                <td valign="top">
                    <table id="tablaCapsulas" name="tablaCapsulas" class="tabla" >
                        ...
                        if ($evaluacionCapsulas) {
                            while ($evaluacionC = mssql_fetch_array($evaluacionCapsulas)) {
                                ?>
                                <tr>
                                    <td>
                                        <input id="<? echo $count ?>" type='checkbox'/>
                                        <input id='capsulaId-<? echo $count ?>' type='hidden' value='<? echo htmlentities($evaluacionC['capsulaId']) ?>'/>
                                    </td>
                                    <td>
                                        <?php echo htmlentities($evaluacionC['capsulaNombre']) ?>
                                    </td>
                                    <td>                                    
                                        <input id="pond-cap-<? echo $count ?>" type="text" style="width:40px; text-align:right" onkeypress="return evaSoloNumeros(event);" onBlur="evaSoloNumerosEnteros(this.id); evaCalcularPonderacion(this.id)" maxlength="3" value="<?php echo htmlentities($evaluacionC['ponderacion']) ?>"/> %
                                    </td>                                
                                </tr>                                
                                <?
                                $count = $count + 1;
                ...
                <td valign="top">
                    <table id="tablaPracticas" name="tablaPracticas" class="tabla" >
                        ...
                        if ($evaluacionPracticas) {
                            while ($evaluacionP = mssql_fetch_array($evaluacionPracticas)) {
                                ?>
                                <tr>
                                    <td>                                    
                                        <input id='prac-<? echo $count ?>' type='checkbox'/><input id='practicaId-<? echo $count ?>' type='hidden' value='<?php echo htmlentities($evaluacionP['practicaId']) ?>'/>
                                    </td>
                                    <td>
                                        <input id='item-<? echo $count ?>' type='text' style='width:340px;' value="<?php echo htmlentities($evaluacionP['practicaNombre']) ?>"/>                                    
                                    </td>
                                    <td>
                                        <input id='pond-item-<? echo $count ?>' type='text' style='width:40px; text-align:right' onkeypress='return evaSoloNumeros(event);' onblur='evaSoloNumerosEnteros(this.id); evaCalcularPonderacion(this.id)' value="<?php echo htmlentities($evaluacionP['ponderacion']) ?>"/>%                                    
                                    </td>                                
                                </tr>                                
                                <?
                                $count = $count + 1;
            ...
            <div style="max-height:300px;width:1010px;overflow:hidden;overflow-y:auto;">
                <table id="listaUsuariosSeleccionados" border=0" class="tabla" style="width:990px;cursor:default; margin:0px">
                    ...
                    <tbody id="listaUsuariosSeleccionadosBody">
                        <?
                        if ($evaluacionUsuarios) {
                            while ($evaluacionU = mssql_fetch_array($evaluacionUsuarios)) {
                                ?>
                                <tr id="fila-<?php echo $evaluacionU['usuarioId'] ?>">
                                    <td style='width:33px;' align="center"><input type='checkbox'/><input type='hidden' id='usuarioId' value='<?php echo htmlentities($evaluacionU['usuarioId']) ?>'/></td>
                                    <td style='width:147px;'><?php echo htmlentities($evaluacionU['nombre']) ?></td>                                                                                          
                                    <td style='width:147px;'><?php echo htmlentities($evaluacionU['usuarioEmail']) ?></td>
                                    <td style='width:147px;'><?php echo htmlentities($evaluacionU['organizacionNombre']) ?></td>
                                    <td style='width:147px;'><?php echo htmlentities($evaluacionU['sectorNombre']) ?></td>
                                    <td style='width:147px;'><?php echo htmlentities($evaluacionU['areaNombre']) ?></td>
                                    <td style='width:147px;'><?php echo htmlentities($evaluacionU['cargoNombre']) ?></td>                                                                                  
                                </tr>      
                                <?
                            }
                        }
    ...
    <table>
        <tr>
            <td width="400px">
                <p>
                    <br>
                    <input type="button" name="button" class="btn azul" value="Guardar" onclick="evaValidarFormularioEvaluacionEdicion()" />                                
                    <label></label>                                
                    <input type="button" name="cancelar" class="btn" id="cancelar" value="Cancelar" onclick="location.href = 'evaEvaluaciones.php'" />
                    <input type="submit" name="submit" id="btnEditarEva" style="display: none" onclick="evaEditarEvaluacion()" />
                </p>
...
mssql_free_result($evaluacionPracticas);
mssql_free_result($evaluacionUsuarios);

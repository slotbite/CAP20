...
$consulta = $objEvaluacion->evaBuscarEvaMantenedor($clienteId, $administradorId, $perfilId);

if ($consulta) {

    $count = 0;

    $lista = "<table id='tablaListaEva' class='tabla'><tbody>";
    //$lista.= '<tr><th></th><th>Tema</th></tr></thead>';

    while ($row = $base_datos->sql_fetch_assoc($consulta)) {
        $lista.= "<tr><td style='width:16px;'><input type='radio' name='listaEva'/></td>";

...
$grupoId = htmlspecialchars(trim($_POST['grupoId']));

$consulta = $objGrupo->manMostrarGrupo($grupoId);

if ($consulta) {

    $grupo = mssql_fetch_array($consulta);

    $grupoNombre = $grupo['grupoNombre'];
    $grupoDescripcion = $grupo['grupoDescripcion'];
    $grupoEstado = $grupo['grupoEstado'];
}

$grupoUsuarios = $objGrupo->manMostrarGrupoUsuarios($grupoId);
?>
...
<input type="hidden" name="grupoId" id="grupoId" value="<?php echo htmlentities($grupoId) ?>"/>
<input type="hidden" name="grupoEstado" id="grupoEstado" value="<?php echo htmlentities($grupoEstado) ?>"/>
    ...
    <div id="contenedor">
        ...
        <div class="divFormulario">
            ...
            <table>
                ...
                <tr>
                    <td>Grupo <span class="requerido">&nbsp;*</span>:</td><td><input type="text" name="updGrupoNombre" id="updGrupoNombre" class="campo" style="text-transform:uppercase;" maxlength="50" value="<? echo htmlentities($grupoNombre) ?>"/> <label id="oGrupo" style="color: #FF0000; font-size: 12px">Debe ingresar un nombre para el Grupo</label></td>
                </tr>                    
                <td>Descripción:</td><td><input type="text" name="updGrupoDescripcion" id="updGrupoDescripcion" class="campoDescripcion" maxlength="250" value="<? echo htmlentities($grupoDescripcion) ?>"/></td>
                </tr>
                ...
                <div style="max-height:300px;width:1010px;overflow:hidden;overflow-y:auto;">
                    <table id="listaUsuariosSeleccionados" border=0" class="tabla" style="width:990px;cursor:default; margin:0px">
                        ...
                        <tbody id="listaUsuariosSeleccionadosBody">
                            <?
                            if ($grupoUsuarios) {
                                while ($grupo = mssql_fetch_array($grupoUsuarios)) {
                                    ?>
                                    <tr>
                                        <td style="width:28px" align="center"><input type='checkbox'/><input type='hidden' id='usuarioId' value='<? echo htmlentities($grupo['usuarioId']) ?>'/></td>
                                        <td style="width:179px"><? echo htmlentities($grupo['nombre']) ?></td>                                        
                                        <td style="width:179px"><? echo htmlentities($grupo['organizacionNombre']) ?></td>
                                        <td style="width:179px"><? echo htmlentities($grupo['sectorNombre']) ?></td>
                                        <td style="width:179px"><? echo htmlentities($grupo['areaNombre']) ?></td>
                                        <td style="width:179px"><? echo htmlentities($grupo['cargoNombre']) ?></td>       
                                        <?
                                    }
                                }
                                ?> 
        ...
        <div class="divFormulario">    
            ...
            <table>          
                <tr>
                    ...
                    <td display="border: none"><input id="gruOrganizacionNombre" name="gruOrganizacionNombre" type="text" class="campo"/></td>
                    <td width="20px"><img id="lupaOrg" src='../skins/saam/img/lupa.png' style="cursor: pointer" onchange="manLimpiarInputs('gruSectorNombre','gruAreaNombre','')" onclick="manSeleccionarOrganizaciones()"/></td>
                    <td valign="top">
                ...
                <tr>
                    ...
                    <td display="border: none"><input id="gruSectorNombre" name="gruSectorNombre" type="text" onfocus="manMostrarSectores('gruOrganizacionNombre', 'gruSectorNombre')" class="campo" maxlength="50"/></td>
                    <td width="20px"><img id="lupaSec" src='../skins/saam/img/lupa.png' style="cursor: pointer" onchange="manLimpiarInputs('gruAreaNombre','','')" onclick="manSeleccionarSectores('gruOrganizacionNombre')"/></td>
                    <td valign="top">
        ...
        <table>
            <tr>
                <td width="400px">
                    <p>
                        <br>
                        <input type="button" name="button" value="Guardar" class="btn azul" onclick="manValidarFormularioGruposEdicion('updGrupoNombre','btEditarGru')" />                                
                        <label></label>                                
                        <input type="button" name="cancelar" id="cancelar" value="Cancelar" class="btn" onclick="location.href = 'manGrupos.php'" />
                        <input type="submit" name="submit" id="btEditarGru" style="display: none" onclick="manEditarGrupo('updGrupoNombre','updGrupoDescripcion')"/>
                    </p>

...
$fileName2 = 'RepGerencia_MAZATLAN.xls';
...
$objPHPExcel->setActiveSheetIndex(2)
            ...
            ->setCellValue('N1','ORGANIZACION')
            ->setCellValue('O1','SECTOR')
            ->setCellValue('P1','AREA');

//CORRECTAS
$query="EXEC listarDatosReporte_MAZATLAN '" . $planificacionFecha . "', 1";
...
$result = $base_datos->sql_query($query);
	...
	while ($row = $base_datos->sql_fetch_assoc($result)) {
		...
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $indice, utf8_encode($row["NOMBRE_USUARIO"]));
                ...
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9, $indice, stripslashes(utf8_encode($row["RESPUESTA"])));
...
$query="EXEC listarDatosReporte_MAZATLAN '" . $planificacionFecha . "',2";
$result = $base_datos->sql_query($query);
	$indice=2;
	$valores="'";
	while ($row = $base_datos->sql_fetch_assoc($result)) {
...
$query="EXEC listarDatosReporte_MAZATLAN '" . $planificacionFecha . "',3";
$result = $base_datos->sql_query($query);
	$indice=2;
	$valores="'";
	while ($row = $base_datos->sql_fetch_assoc($result)) {
...
$query="EXEC listarDatosReporte_MAZATLAN_Resumen '" . $planificacionFecha . "'";
$result = mssql_query($query);

if(mssql_num_fields($result) > 0){
    ...
    for ($i = 1; $i < $n_campos; ++$i) {
        ...
        //$formula='=' . $letra . '2+'. $letra .'3+' . $letra . '4';
...
//        $formula='=(' . $letra.'2+'.$letra.'3)/'.$letra.'5';
//        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($i, $indice+1,$formula );
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($i, $indice+1,$valorcelda);

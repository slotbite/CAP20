...
$fileName2 = 'RepFlota_Remolcadores.xls';
...
//CORRECTAS
$query="EXEC listarDatosReporte_RAM $mm,$yyyy,1";
...
$query="EXEC listarDatosReporte_RAM $mm,$yyyy,2";
$result = $base_datos->sql_query($query);
	$indice=2;
	$valores="'";
	while ($row = $base_datos->sql_fetch_assoc($result)) {
		...
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $indice, utf8_encode($row["NOMBRE_USUARIO"]));
                ...
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $indice, $row["CAPSULA"]);
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $indice, $row["ID_PREGUNTA"]);
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $indice, stripslashes(utf8_encode($row["PREGUNTA"])));
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $indice, $row["ID_RESPUESTA"]);
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(9, $indice, $row["RESPUESTA"]);
...
$query="EXEC listarDatosReporte_RAM $mm,$yyyy,3";
$result = $base_datos->sql_query($query);
	$indice=2;
	$valores="'";
	while ($row = $base_datos->sql_fetch_assoc($result)) {
		...
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $indice, utf8_encode($row["NOMBRE_USUARIO"]));
                ...
                $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(10, $indice)->getNumberFormat()->setFormatCode('dd/mm/yyyy');
                
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, $indice, $row["FECHA_ENVIO"]);
                $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(11, $indice)->getNumberFormat()->setFormatCode('dd/mm/yyyy');
                
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(12, $indice, $row["FECHA_CIERRE"]);
                $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow(12, $indice)->getNumberFormat()->setFormatCode('dd/mm/yyyy');
...
$query="EXEC listarDatosReporte_RAM_Resumen ";
...
for ($i = 1; $i < $n_campos; ++$i) {
 ...
 $valorcelda=$objPHPExcel->getActiveSheet()->getCellByColumnAndRow($i, $indice+1)->getCalculatedValue();
 ...
 $queryColores 	= "EXEC CapXLSColores ".$valorcelda."";
	...
	$rowColores	= $base_datos->sql_fetch_assoc($resultColores);
        ...
        $color=$rowColores['color'] ? $rowColores['color']:'';
 ...
 $objPHPExcel->getActiveSheet()->getStyle($letra.($indice+1))->applyFromArray($colores);
 ...
 $objPHPExcel->getActiveSheet()->getStyle("A8:C11")->applyFromArray($styleArray);
 $objPHPExcel->getActiveSheet()->setCellValue('A8','Indicador');
 $objPHPExcel->getActiveSheet()->setCellValue('B8','Desde');
 $objPHPExcel->getActiveSheet()->setCellValue('C8','Hasta');
 
 $objPHPExcel->getActiveSheet()->getStyle("A8:C8")->applyFromArray(array("font" => array( "bold" => true)));
 
  $objPHPExcel->getActiveSheet()->setCellValue('A9','VERDE');
  $objPHPExcel->getActiveSheet()->setCellValue('A10','AMARILLO');
  $objPHPExcel->getActiveSheet()->setCellValue('A11','ROJO');
...
$indice=9;
 while ($rowSem = mssql_fetch_array($resultSem)) {
    ...
    $objPHPExcel->getActiveSheet()->getStyle('A'.$indice)->applyFromArray($colores);
    //$objPHPExcel->getActiveSheet()->setCellValue('A'.$indice,utf8_encode($rowSem[0]).'%');
    $objPHPExcel->getActiveSheet()->setCellValue('B'.$indice,utf8_encode($rowSem[1]).'%');
